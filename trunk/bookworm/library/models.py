...
aise Invalidtocid:
        ...
        on("Could not find an item matching %s in OPF <item> liany NCX file. EpubCheck 1.0.3 may erroneously report this as valid.
...
class Subject(BookwormModel):
    ...
    def __unicode__(self):
        return selclass Meta:
        ordering = ('name',)n self.name
        
class EpubPublisher(BookwormModel):
    '''Represents a publisher'''
    name = models.CharField(max_length=255, default='', db_i
    def __unicode__(self):
        return self.name
...
xception()
    ...
    def _clean_xhtml(self, xhtml):
        ...
        for element in xhtml.getiterator():
            ...
            if element.tag ==element.get('src') is not None and 'svg' in element.get('src'):
                    p = element.getparent()         
    )         
...
=True)
    ...
    def get_api_key(self):
        ...
        return smart_str(APIKey.objects.get_or_create(user=self.user)[0].key, 'utf8')er.username
...
class SystemInfo():
    ...
    def get_total_books(self'''If there are enough books, round the total so users don't
...
class BinaryBlob(B, StoragBookwormFile):
    ...
    def get_data(self):
        ...
        if not os.path.exists(f.encode('utf8')):
            ...
            return None
        return open(f.enc, 'rb+'ode('utf8')).read(get_data_handler(self):
        '''Return the data for this file, as a filehandlerom read())'''
        f = self._get_file()
        if not os.path.exists(f.encode('utf8')):
            log.warn(u"Tried to open file %s but it wasn't there (storage dir %s)" % (f, self._get_storage()))
            return None
        return open(f.enc, 'rb+'o
